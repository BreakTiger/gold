...
Page({
    ...
    http.sendRequest('huishou.userma', 'post', data).then(function (res) {
      ...
      if (res.error == 0) {
        ...
        if (detail.shop.shopname) {
          console.log('店铺')
          that.canvas_one(detail)
        } else {
          console.log('个人')
          that.canvas_two(detail)
        }
  ...
  // 海报一
  canvas_one: async function (detail) {
    const ctx = wx.createCanvasContext('shareCanvas');
    ctx.drawImage('../../img/poster.png', 0, 0, 347.5, 565.5);
          ...
          success: function (res) {
            ...
            wx.downloadFile({
              ...
              success: function (res) {
                ...
                ctx.fillText('电话：' + detail.shop.shopmobile, 20, 300, 296)
                ...
                ctx.setFillStyle('#000000') //文字颜色
                ctx.fillText(detail.shop.shopname, 20, 270, 296)
  ...
  // 海报二
  canvas_two: async function (detail) {
    const ctx = wx.createCanvasContext('shareCanvas');
    ctx.drawImage('../../img/poster_one.png', 0, 0, 347.5, 565.5);
    await wx.downloadFile({
      url: detail.avatar,
      success: function (res) {
        let img1 = res.tempFilePath
        wx.downloadFile({
          ...
          success: function (res) {
            ...
            ctx.setFillStyle('#FF5E10') //字体颜色
            ctx.textAlign = "center"; //文字居中
            ctx.fillText(detail.nickname, 175, 160)

            // 用户联系
            ctx.setFontSize(12) //文字大小
            ctx.setFillStyle('#666666') //文字颜色
            ctx.fillText('联系方式:' + detail.mobile, 175, 200)

            //经营范围
            ctx.setFontSize(15) //文字大小
            ctx.setFillStyle('#000000') //文字颜色
            ctx.fillText('经营范围：', 75, 240)
            ...
            for (var b = 0; b < row.length; b++) {
              ctx.fillText(row[b], 90, 280 + b * 30, 600);
            }

            // 提示
            ctx.setFontSize(13) //文字大小
            ctx.setFillStyle('#000000') //文字颜色
            ctx.fillText('长按小程序识别', 175, 520)

            //分享码
            ctx.drawImage(img2, 130, 380, 100, 100);
